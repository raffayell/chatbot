...
async def translate_to_armenian(msg):
    messages = [
        ("system", "Translate the user sentence to Armenian."),
        ("human", msg),
    ]
...
def Title() -> ft.Text:
    return ft.Text(
        "Make your own Chat Application with Ollama and Langchain",
...
def MessageForm(add_message: callable) -> ft.Column:
    ...
    async def on_click(e: ft.Event) -> None:
        if msg.strip():
            add_message("me", msg)
            set_msg("")
            answer = await translate_to_armenian(msg)
